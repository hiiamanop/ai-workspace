...
**Complexity classification is a small local HF model, not an LLM call.**
...
doubling the cost of every turn. It now calls MADE's `POST /classify`
(`core/complexity.py`, `BCN001/llm-complexity-router`, deberta-v3-small,
~100M params, ~30-60ms on CPU) which correctly separates prompt *length*
from task *difficulty*. Falls back to `"medium"` if MADE is unreachable.
...
**Markdown→Rego compile is LLM-based (deliberately reverted from a deterministic parser).** `src/openwebui-policy-compiler.ts`'s `compilePolicy()` sends the Markdown to DeepSeek (`deepseek-v4-flash`, the cheapest tier — it is also a valid raw DeepSeek API model id, passed through as-is) and asks it to emit OPA Rego `deny[]` rules under `package made.hard`. Compilation is NOT deterministic — free-text-to-Rego is an LLM call — but the output is gated so nothing silently-wrong reaches MADE: it throws a `CompileError` on an empty policy, on an `UNABLE_TO_COMPILE:` refusal, on output missing `package made.hard`, and on output defining `default allow` (MADE's `base.rego` owns that; a second `default` breaks the whole hard-constraint engine). A policy whose Rego has no `deny[]` rules compiles with a warning (it won't block anything). This used to be a deterministic regex/grammar parser; reverted because admin-authored policy prose is free-form and the LLM path (with these guards) is more useful than a strict fixed syntax.

**Chat-assisted policy drafting (LLM writes the Markdown, and the same LLM validates it):** `openwebui/src/lib/components/workspace/Policies/PolicyDraftChat.svelte` is a chat panel embedded in both the `new` and `[id]` policy pages (hidden when a policy is active/locked) — type a request in plain language, or upload a regulation document (`.txt/.md/.pdf/.docx`, extracted via Open WebUI's own existing file-upload pipeline, no new parser built for this) and it's attached to the next message. Backed by `src/policy-drafter.ts`'s `draftPolicy()` (DeepSeek `deepseek-v4-flash` call) and proxied through `POST /api/v1/policies/draft` (admin-only, stateless — full message history resent each turn, nothing persisted server-side) in `open_webui/routers/policies.py`. The system prompt teaches the model a structured `IF/THEN deny` rule syntax that turns into clean, reliable `deny[]` rules (the compiler tolerates prose, but this form is most reliable); critically, `draftPolicy()` runs the real LLM `compilePolicy()` against any fenced draft the model produces **before** reporting it as ready — a draft that doesn't actually compile is never surfaced as a usable draft, its `CompileError` message is appended to the chat reply instead so the next turn can ask the model to fix that exact rule. This costs a second cheap `deepseek-v4-flash` call per fenced draft. The extracted draft only ever fills the page's Markdown textarea for human review — it still has to pass through the same compile/deploy buttons as any hand-written policy.
...
**MADE's privacy subsystem (`MADE/core/privacy/`):** `POST /privacy/redact` / `POST /privacy/restore` — reversible pseudonymization, not a vector-DB/RAG feature (that was scoped out, see `docs/PRD-confidentiality-pipeline.md`'s "No vector store" note). `detectors.py` combines regex with NER, earlier passes winning on overlap (regex > Indonesian NER > English NER):
- **regex** (`detect()`): email, phone (incl. `+62`/`08xx`), card-number (Luhn-validated), NIK (16-digit `ID_NUMBER`), plus Indonesian NPWP / KK / vehicle-plate — the digit-ambiguous ones (`KK`, bare `NPWP`) are only claimed when a keyword *precedes* the number (`_keyword_precedes`, left-only: a keyword *after* is the next field's label). Also `SECRET` (API keys, AWS `AKIA`, JWT, private-key headers, `password:`-style) — redacted to a **non-reversible** `[SECRET_REDACTED]` marker, never stored even encrypted.
- **NER**: `en_core_web_sm` (English) + `cahya/bert-base-indonesian-NER` (Indonesian, `MADE_ID_NER_MODEL` to override, baked into the image, **fail-soft** — regex+English only if it can't load). `_add_ner_spans` drops spans that start/end mid-word (subword-tokenizer garbage).

`pseudonymizer.py` persists placeholder↔original mappings in `entity_mappings` (keyed `(org_id, sha256(original_value))`; Fernet-encrypted, **required** `MADE_MAPPING_ENCRYPTION_KEY`, else `MissingEncryptionKeyError`). After each `redact()` it re-runs regex `detect()` on its own output — a surviving span logs a `redaction_leaks` row (hash only, never the text) and, if `MADE_REDACTION_LEAK_FAIL_CLOSED=1`, raises `RedactionLeakError`. `restore()` warns on model-mangled placeholders (`[Person 3]`) and recovers them if `MADE_RESTORE_FUZZY=1`.

**Auto data-classification (`POST /privacy/classify`, `classifier.py`):** decides `public|internal|confidential|restricted` for a piece of text — cache → heuristic → cheap-LLM tie-break only when the heuristic is inconclusive. Heuristic precedence: `SECRET` span → `restricted`; any ID span → `confidential`; ≥2 PERSON + 1 ORG → `confidential`; `lexicon.yaml` phrase (EN/ID: "rahasia", "gaji", "akuisisi", …) → `confidential`/`public`; contact span or short text → `internal`; else inconclusive → LLM. LLM (`llm_client.py`, `MADE_CLASSIFIER_API_KEY`/`_MODEL`/`_BASE_URL`, **MADE's only model-provider secret**) **never raises** — no key / timeout / bad output → `internal`@0.4. Results cached in `classification_cache` per `(org_id, sha256(text))`. `restricted.rego` denies external vendors for `restricted` data **even when redacted** (a leaked secret is a leaked secret).

**Entity-mapping admin (Phase D):** `GET/PATCH/DELETE /privacy/mappings` + `GET /privacy/leaks` on MADE (internal network only, `org_id` param). Fronted by an admin-only Open WebUI backend router (`open_webui/routers/privacy.py`, `require_admin` + httpx forward, `org_id = user.id`) and a workspace tab (`(app)/workspace/privacy-mappings/` — table with click-to-reveal originals, inline edit, delete, plus a redaction-leaks section). `PATCH` re-encrypts + rehashes on value change (409 on collision), validates placeholder shape (400).

**Confidential-redaction Filter (Phase 3, Open WebUI's chat path):** `openwebui-filters/confidential_redaction.py` is a second Filter (separate from `made_routing.py`) whose `inlet()` calls MADE's `/privacy/redact` on every user message and whose `outlet()` calls `/privacy/restore` on every message in the response. `org_id` for the per-org entity-mapping scope is the requesting user's id (`__user__.id` — no auth token needed, same as any Tool). When it actually redacts something it sets `body["_privacy"] = {"data_classification": "confidential", "redacted": True}`; `made_routing.py`'s `_call_made()` reads that (falling back to `"internal"`/not-redacted when absent, so it still works standalone) and forwards it to MADE's `/decide`, which is what lets `external_vendor.rego`'s deny rule actually engage. Must run before `made_routing.py` for that signal to exist in time — `Valves.priority` defaults to `-10` (Open WebUI sorts filters ascending by priority; `made_routing.py` defaults to `0`). Provisioned by `src/openwebui-provision-redaction.ts`'s `provisionRedactionFilter()` — simpler than `provisionFilter()` since this Filter's valves are just `MADE_URL`/`REQUEST_TIMEOUT_SECONDS` (it never calls back into Open WebUI's own API, so no admin API key is needed at all — though the reconciler still passes it the shared token, to reuse the signin from minting rather than sign in a fourth time per cycle). Wired into `src/openwebui-provisioning-reconciler.ts`'s `reconcileOnce()` alongside the other two provisioners.

`confidential_redaction.py`'s `inlet()` now also calls `POST /privacy/classify` on the combined user text: it **always** sets `body["_privacy"]` when the result is confidential/restricted (even with nothing redactable — sensitive prose with no PII still must not leave unredacted), and degrades to redact-only if `/classify` is unreachable. When `/decide` then approves no model for a sensitive request, `made_routing.py` raises `BLOCKED_MESSAGE` instead of silently falling back to a cheap external tier. `src/chat.ts` **is** now wired for the same classify→redact→decide→restore flow (`src/privacy-client.ts`, `CHAT_ORG_ID = "anonymous"` since that page has no auth user) — streamed deltas are buffered when redacted (placeholders can split across deltas) and the restored text emitted once.
